    ...
    if (existsSync(srcFullPath)) {
      ...
      for (const file of files) {
        ...
        if (stats.isDirectory()) {
          returnFiles.push(...getBloomFilesRecursively(dataDir, join(src, file), fullDest));
        }
      }
      return returnFiles;
    } else {
...
function convertBloomBook(
  ...
  if (!distExists) {
    ...
    files.push({
      path: path.join("src", "gen-assets", "collections", context.bcId, book.id, ".distribution"),
      content: "bloom-web",
    });
  }
  ...
  if (verbose >= 3) console.log(`Save bloom html file for ${book.name} --> ${book.hashedFileName}`);
  files.push({
    path: join(
      "src",
      "gen-assets",
