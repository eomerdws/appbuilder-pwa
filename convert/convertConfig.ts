  ...
  for (const tag of styleTags) {
    ...
    for (const propertyTag of propertyTags) {
      ...
      if (propName && propValue) {
        ...
        if (verbose) {
          console.log("Parsing " + propName + " = " + propValue + " -> " + properties[propName]);
        }
      }
    }
    styles.push({
...
export function parseStylesInfo(stylesInfoTag: Element, verbose: number): StyleConfig {
  return {
    font: stylesInfoTag.getElementsByTagName("text-font")[0].attributes.getNamedItem("family")!
      .value,
    lineHeight: parseInt(
      stylesInfoTag.getElementsByTagName("line-height")[0].attributes.getNamedItem("value")!.value,
    ),
    ...
    textDirection: stylesInfoTag
      ...
      .attributes.getNamedItem("value")!.value,
    textSize: parseInt(
      stylesInfoTag.getElementsByTagName("text-size")[0].attributes.getNamedItem("value")!.value,
    ),
...
async function convertConfig(dataDir: string, verbose: number) {
  ...
  // Version
  data.version = document.getElementsByTagName("version")[0].attributes.getNamedItem("name")!.value;
  ...
  if (isSAB(data)) {
    ...
    data.traits["has-glossary"] =
      data.bookCollections.filter((bc) => bc.books.filter((b) => b.type === "glossary").length > 0)
        .length > 0;
  }
  if (isDAB(data)) {
      ...
      colorSets: Array.from(colorSetTags).map((cst) => {
        ...
        for (const color of colorTags) {
          ...
          if (name && value) {
            ...
            if (verbose >= 3) {
              console.log(`.. skipping deprecated color "${name}" in "${theme}" theme.`);
            }
            continue;
          } else if (name && !value) {
...
export function parseBookCollections(document: Document, dataDir: string, verbose: number) {
  ...
  for (const tag of booksTags) {
    ...
    for (const book of bookTags) {
      ...
      const bookFeaturesTag = book.querySelector("features[type=book]")?.getElementsByTagName("e");
      const bookFeatures: any = {};
      if (bookFeaturesTag) {
        for (const bookFeature of bookFeaturesTag) {
          bookFeatures[bookFeature.attributes.getNamedItem("name")!.value] = parseConfigValue(
            bookFeature.attributes.getNamedItem("value")!.value,
          );
        }
      }
      const quizFeaturesTag = book.querySelector("features[type=quiz]")?.getElementsByTagName("e");
      ...
      if (quizFeaturesTag) {
        ...
        for (const quizFeature of quizFeaturesTag) {
          if (quizFeature.attributes.getNamedItem("name")!.value === "access-code") {
            ...
            quizFeatures[quizFeature.attributes.getNamedItem("name")!.value] = parseConfigValue(
              quizFeature.attributes.getNamedItem("value")!.value,
            );
          }
        }
      }
        ...
        for (const bookTab of bookTabsTag.getElementsByTagName("book-tab")) {
          ...
          if (tabFeaturesTag) {
            for (const tabFeature of tabFeaturesTag) {
              tabFeatures[tabFeature.attributes.getNamedItem("name")!.value] = parseConfigValue(
      ...
      books.push({
        portions: book.getElementsByTagName("portions")[0]?.attributes.getNamedItem("value")?.value,
        chapters,
        chaptersN,
        chaptersLabels,
        fonts,
        ...
        hashedFileName:
          format !== undefined && ["html", "bloom-player"].includes(format)
    ...
    if (verbose >= 3) {
      console.log(`.... collection: `, JSON.stringify(bookCollections[bookCollections.length - 1]));
    }
  }
  ...
  for (const translationMappingsTag of translationMappingsTags) {
    ...
    for (const tag of translationMappingsTags) {
      ...
      for (const localization of tag.getElementsByTagName("t")) {
        localizations[parseLangAttribute(localization)] = decodeFromXml(localization.innerHTML);
      }
      if (verbose >= 3) {
...
export async function verifyMediaAvailability(config: AppConfig, verbose: number) {
  ...
  for (const [key, source] of sources.entries()) {
    ...
    if (verbose) {
      if (source.media.length) {
        ...
        console.log(` ⚠️ No media included for source ${key} (${source.type}) - ${source.name}`);
      }
    }
  ...
  for (const [key, source] of sources.entries()) {
    if (source.errored) {
      config.audio!.sources[key].accessMethods = config.audio!.sources[key].accessMethods?.filter(
        (m) => m !== "download",
      );
      console.log(
    ...
    for (const tag of imagesTags) {
      if (tag.getAttribute("type") === "illustration") {
        ...
        for (const image of illustrationTags) {
          const filename = image.getElementsByTagName("filename")[0]?.innerHTML || image.innerHTML;
          const width = image.getAttribute("width") ? parseInt(image.getAttribute("width")!) : 0;
          const height = image.getAttribute("height") ? parseInt(image.getAttribute("height")!) : 0;
...
export function parseMenuItems(document: Document, type: string, verbose: number) {
  ...
  if (menuItemTags && menuItemTags?.length > 0) {
    for (const menuItem of menuItemTags) {
      ...
      const linkIdTags = menuItem.getElementsByTagName("link-id")[0]?.getElementsByTagName("t");
